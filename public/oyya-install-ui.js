(()=>{
 ...
 function showInstructions(){
   ...
   const title=isIOS?'تثبيت OYYA على الآيفون':isAndroid?'تثبيت OYYA على الهاتف':'تثبيت OYYA كتطبيق';
   const steps=isIOS
    ?['افتح OYYA من Safari.','اضغط زر المشاركة في أسفل Safari.','اختر «إضافة إلى الشاشة الرئيسية».','اضغط «إضافة» وسيظهر OYYA كتطبيق مستقل.']
    :isAndroid
   ...
   m.innerHTML=`<div class="oyya-install-sheet" role="dialog" aria-modal="true"><button class="oyya-install-x" type="button">×</button><div class="oyya-install-mark">OYYA</div><h2>${title}</h2><p>لا نستخدم نوافذ المتصفح القديمة. هذه هي طريقة التثبيت الصحيحة لهذا الجهاز.</p><div class="oyya-install-steps">${steps.map((s,i)=>`<div><b>${i+1}</b><span>${s}</span></div>`).join('')}</div><button class="oyya-install-done" type="button">حسنًا</button></div>`;
   document.body.appendChild(m);document.body.classList.add('oyya-install-open');
   m.querySelector('.oyya-install-x').onclick=close;m.querySelector('.oyya-install-done').onclick=close;m.onclick=e=>{if(e.target===m)close()};
 }
